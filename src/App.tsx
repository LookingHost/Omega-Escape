...
export default function App() {
  ...
  useEffect(() => {
    ...
    const onDown = (e: PointerEvent) => {
      ...
      // hold-to-dash on the canvas only when on-screen buttons are hidden
      if (settingsRef.current.swipeControls) {
    ...
    const onMove = (e: PointerEvent) => {
      ...
      if (!settingsRef.current.swipeControls) return; // swipe gestures only in swipe mode
    ...
    const onUp = (e: PointerEvent) => {
      ...
      engineRef.current?.slideEnd(); // release any held slide from a swipe
    ...
    <div className="safe-pad pointer-events-none absolute inset-0 z-20 flex flex-col justify-between p-3 sm:p-5">
      ...
      <div className="flex items-end justify-between gap-2">
        <div className="flex flex-col gap-1.5">
          {hud.objective && (
            <div className={cn("w-56 max-w-[60vw] rounded-md border px-3 py-1.5 backdrop-blur-md transition",
...
function Briefing({ char, mode, best, onLaunch }: {
  ...
  useEffect(() => {
    ...
    const tick = () => {
      if (launchedRef.current) return;
      setPhase(seq[i]);
      if (seq[i] === "go") { audio.perfect(); timers.current.push(window.setTimeout(launchOnce, 450)); return; }
      if (typeof seq[i] === "number") audio.click();
      const wait = times[i];
      i++;
      timers.current.push(window.setTimeout(tick, wait));
    };
    // If hidden during countdown, pause it; restart the countdown when visible.
    const onVis = () => {
      if (launchedRef.current) return;
      if (document.hidden) {
        timers.current.forEach(clearTimeout);
    ...
    return () => { t.forEach(clearTimeout); document.removeEventListener("visibilitychange", onVis); };
    // eslint-disable-next-line react-hooks/exhaustive-deps
  }, []);

  return (
    <div
      className="absolute inset-0 z-40 flex items-center justify-center overflow-hidden bg-[#070312]"
      onPointerDown={launchOnce}
    >
      ...
      <div className="pointer-events-none absolute bottom-6 left-0 right-0 text-center text-[11px] font-bold uppercase tracking-[0.3em] text-slate-500">
        Tap to skip
      </div>
    </div>
  );
        ...
        <div className="flex flex-col justify-between gap-4">
          <div className="relative">
            <div className="relative inline-block">
              <h1 className="font-display text-6xl font-black leading-[0.9] tracking-tighter text-white sm:text-8xl"
                style={{ textShadow: "0 0 30px rgba(34,211,238,0.55), 3px 3px 0 #a21caf" }}>
                OMEGA
              </h1>
              <div className="font-display -mt-1 pl-2 text-3xl font-black tracking-[0.55em] text-orange-400 sm:text-4xl"
                style={{ textShadow: "0 0 22px rgba(249,115,22,0.8)" }}>
                ESCAPE
              </div>
              <div className="absolute -left-3 top-0 h-full w-1 bg-gradient-to-b from-cyan-400 via-fuchsia-500 to-transparent" />
            </div>
  ...
  return (
    <Overlay>
      <div className="w-full max-w-md">
        ...
        <Panel className="flex max-h-[78vh] flex-col gap-2 overflow-y-auto p-5">
          {items.map((it) => (
